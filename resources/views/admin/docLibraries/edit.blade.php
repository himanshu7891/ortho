...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.docLibrary.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.doc-libraries.update", [$docLibrary->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="doc_name">{{ trans('cruds.docLibrary.fields.doc_name') }}</label>
                <input class="form-control {{ $errors->has('doc_name') ? 'is-invalid' : '' }}" type="text" name="doc_name" id="doc_name" value="{{ old('doc_name', $docLibrary->doc_name) }}" required>
