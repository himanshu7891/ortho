...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.casesModal.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cases-modals.update", [$casesModal->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="patient_first_name">{{ trans('cruds.casesModal.fields.patient_first_name') }}</label>
                <input class="form-control {{ $errors->has('patient_first_name') ? 'is-invalid' : '' }}" type="text" name="patient_first_name" id="patient_first_name" value="{{ old('patient_first_name', $casesModal->patient_first_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('patient_last_name') ? 'is-invalid' : '' }}" type="text" name="patient_last_name" id="patient_last_name" value="{{ old('patient_last_name', $casesModal->patient_last_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('patient_dob') ? 'is-invalid' : '' }}" type="text" name="patient_dob" id="patient_dob" value="{{ old('patient_dob', $casesModal->patient_dob) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\CasesModal::PATIENT_GENDER_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('patient_gender') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="patient_gender_{{ $key }}" name="patient_gender" value="{{ $key }}" {{ old('patient_gender', $casesModal->patient_gender) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('originator') ? 'is-invalid' : '' }}" name="originator" id="originator" required>
                    ...
                    @foreach($doctors as $id => $doc)
                        <option value="{{ $doc->id }}" {{ (in_array($doc->id, old('originator', [])) || $casesModal->originator_id == $doc->id) ? 'selected' : '' }}>{{ $doc->first_name .' '.$doc->last_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('collaborator') ? 'is-invalid' : '' }}" name="collaborator[]" id="collaborator" multiple required>
                    <option></option>
                    @foreach($doctors as $id => $doc)
                        <option value="{{ $doc->id }}"{{ (in_array($doc->id, old('roles', [])) || in_array($doc->id , $collaborators)) ? 'selected' : '' }}>{{ $doc->first_name .' '.$doc->last_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description" required>{{ old('description', $casesModal->description) }}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('status_id') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="status_id" id="status_id" value="1" {{ $casesModal->status_id || old('status_id', 0) === 1 ? 'checked' : '' }}>
